//
// kernel.h — the Circle kernel that hosts OpenTTD.
//
// Device ownership: this kernel brings up interrupts, the timer, a serial
// console carrying stdio, the SD card (FatFs, holding the base sets, the
// language files and the settings) and the cooperative scheduler. Video, USB
// input and audio belong to circle-libsdl2 and are created inside SDL_Init
// when the game calls it. So do the CPU clock and the case fan, which the
...
//   core 1   the APPLICATION core. OpenTTD, alone: the game loop and
