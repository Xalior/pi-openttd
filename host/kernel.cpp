//
// kernel.cpp — brings the board up, then hands control to OpenTTD.
//
// OpenTTD is an ordinary command-line program: it expects a working standard
// library, a filesystem holding its base sets and language files, and an
...
// `settings.ini` into the card's root would each silently overwrite the
// other's.
//
// Three things point at it, and between them they cover every path the game
// can reach: argv[0] is an absolute path inside it, which is what OpenTTD
// derives its own program directory from; SDL_GetBasePath and
// SDL_GetPrefPath answer with it, which is where a build without a personal
// directory looks for its data and writes its configuration (see
// circle_stubs.cpp); and this kernel makes it the working directory before
...
//
// argv[0] is an absolute path rather than a bare program name: OpenTTD takes
// the directory part of argv[0] as one of the places it looks for its data,
// so this is what makes the base sets, the language files and the script
// compatibility layers findable with nothing else configured.
//
// The three drivers are named rather than left to OpenTTD's own automatic
// choice, because one of the automatic answers would be wrong here:
//
//   -v sdl    the SDL2 video backend. The only other one compiled in is the
//             dedicated-server one, which draws nothing.
//   -s sdl    the SDL2 sound backend, which the shim's audio device serves.
//   -m null   NO music. The only other music backend compiled in plays a
//             MIDI file by starting a separate program, it has the higher
//             priority, and it would therefore be the automatic choice —
//             but there is no second process to start on a bare-metal
//             board.
//
...
// without the card or the build changing — and OpenTTD has a full command
// line, so that covers the resolution, the blitter and a saved game to load
// straight into.
static const char *OpenTTDArgv[] = {
    RAPI_GAME_DIR "/openttd",
    "-v", "sdl",
    "-s", "sdl",
    "-m", "null",
};
...
// terminating null.
static const char *s_FinalArgv[sizeof(OpenTTDArgv) / sizeof(OpenTTDArgv[0])
...
void CSplitCores::Run(unsigned nCore)
{
    ...
    // that throws, and a throw reads this register first. OpenTTD throws in
    // ordinary operation — its savegame reader signals every error that way
    // — so this is not a formality on this payload.
    ...
    case 1:
        ...
        s_AppResult = openttd_unix_main(s_FinalArgc,
...
boolean CKernel::Initialize(void)
{
    ...
    // is plausible. OpenTTD seeds its random generator from it and stamps
    // save games with it.
    ...
    // no keyboard, mouse or pad, and that is worth saying rather than dying
    // for.
...
{
    m_Logger.Write(From, LogNotice, "starting OpenTTD");
    ...
    // the library about the display. Every SDL answer OpenTTD gets — the
    // current mode, the window, the window's surface — is this, whatever the
    // panel is really scanning, and the library carries each frame from the
    // one to the other.
    //
    // 640x480 is the smallest window OpenTTD will run in and the size its
    // own default settings ask for. It is stated here as a build constant
    // because the shim's virtual device is fixed for the whole run: OpenTTD
    // can be told to use a different resolution, but not one the display was
    // never declared at.
    //
    ...
    s_FinalArgc = DefaultsBuildArgv(OpenTTDArgv,
                                    sizeof(OpenTTDArgv) / sizeof(OpenTTDArgv[0]),
                                    s_FinalArgv,
    ...
    // card carries several games, and two of them writing a `settings.ini`
    ...
    // Give the disk cache its pool, now that the block has been read and the
    // two --rapi-cache switches have had their say. Before the game runs, so
    // the one allocation it ever makes happens while the heap is still empty,
    ...
    m_Logger.Write(From, LogNotice, "OpenTTD exited with %d — parked", res);
    for (;;)
    {
