...
// owns the hardware.
//
// THE PROBLEM. OpenTTD reads its base sets, its language files, its
// settings and its saved games through the C++ standard library and plain
// C: std::ifstream, std::filesystem, fopen/fread, opendir/readdir. Those
// reach newlib, and newlib's Circle glue drives FatFs and the SD card
// directly. On this kernel the game runs on the application core, and
// touching a device from any core but core 0 is illegal — the whole Circle
// world, interrupts included, belongs to core 0. The game has no file layer
// of its own to redirect, so there is nothing in it to change.
//
...
// remaining descriptor-table calls are still marshalled with
// SDL2Circle_CallOn0 and run unchanged on core 0.
//
